        ...
        while True:
            ...
            if STATE.cpu_temp >= 75:
                ...
                if time.time() > self.next_whine_time:
                    parietal_lobe.thread.accept_new_message(text=f'(The raspberry pi CPU temperature is now {STATE.cpu_temp} celcius. This is critical. Your body may shut down soon. Please plead for immediate assistance!)')
                    self.next_whine_time = time.time() + 60

            elif STATE.cpu_temp >= 73:
                log.main.warning("This is fine (%sC)", STATE.cpu_temp)
                if time.time() > self.next_whine_time:
                    parietal_lobe.thread.accept_new_message(text=f'(The raspberry pi CPU temperature is now {STATE.cpu_temp} celcius. This is very hot. Please speak a dire warning about this issue.)')
                ...
                if time.time() > self.next_whine_time:
                    parietal_lobe.thread.accept_new_message(text=f'(The raspberry pi CPU temperature is now {STATE.cpu_temp} celcius. This is a bit high. Please speak a warning.)')
